...
var dialog_script = [
	...
	{
		'background': "res://art/fonts/chmorka.png"
	},
	{
...
func event_handler(event):
	match event:
		...
		{'question', ..}:
			...
			#print("f1" , button.text)
			print("f2" , event['variable'])
			print("_on_option_selected")
			print(event)
			print(event['options'])
		...
		{'input', ..}:
			...
			$TextInputDialog.popup()
			$TextInputDialog.connect("confirmed", self, "_on_input_set", [event['variable']])
		{'action'}:
			if event['action'] == 'game_end':
				get_tree().quit()
			if event['action'] == 'end_lv12':
				Save1.end_lv12 = true
			if event['action'] == 'fire':
				Save1.end_lv13_fire = true
			if event['action'] == 'end_lv_13':
				Save1.end_lv13 = true
			if event['action'] == 'lv_up':
				Save1.lv_up = true
			if event['action']=="end_lv14":
				Save1.end_14 = true
			if event['action']=="end_demo":
				Save1.end_demo = true
		{'scene'}:
...
func _on_option_selected(option, variable, value):
	...
	print(global.custom_variables)
	if option.text == "Ataku":
		Save1.odp1 = true
	if option.text == "Pioskol":
		Save1.odp2 = true
	if option.text == "Liść Krali":
		Save1.odp3 = true
